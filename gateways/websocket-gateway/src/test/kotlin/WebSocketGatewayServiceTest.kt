 ...
 */
class WebSocketGatewayServiceTest : KoinTest {
    
    private val gatewayService: WebSocketGatewayService by inject()
    ...
    fun setup() {
        ...
        startKoin {
            modules(
                module {
                    single<WebSocketGatewayService> { WebSocketGatewayServiceImpl() }
                }
            )
        }
    }
    ...
    private fun mockWebSocketSession(): WebSocketSession {
        ...
        return object : WebSocketSession {
            ...
            override suspend fun incoming(): ReceiveChannel {
                // 模拟接收通道
                return Channel<Frame>().apply { close() }
            }
            
            override suspend fun outgoing(): SendChannel {
                // 模拟发送通道
                return Channel<Frame>().apply { close() }
            }
        }
