...
import org.now.terminal.session.domain.Process
...
/**
 * 基于Pty4j的Process适配器实现
 */
class Pty4jProcessAdapter(
    ...
    fun start() {
        try {
            // 构建命令和环境变量
            val command = ptyConfig.command.toTypedArray()
            val environment = ptyConfig.environment.toTypedArray()
            ...
            ptyProcess = PtyProcessBuilder()
                ...
                .setInitialColumns(ptyConfig.initialSize.columns)
                .setInitialRows(ptyConfig.initialSize.rows)
                .start()
